...
check("1. Guaraqueçaba indo pra L = afastando (receding)", c1.v.approach === "receding", `deu ${c1.v.approach}`);

// 2. Caso oposto: núcleo a oeste indo para leste = APROXIMANDO
const c2 = describe("Núcleo a OESTE indo para L (deve APROXIMAR)", -25.1, -52.5, 91, 40);
check("2. Núcleo a Oeste indo pra L = aproximando", c2.v.approach === "approaching", `deu ${c2.v.approach}`);

// 3. Caso tangencial: núcleo ao norte indo para leste
const c3 = describe("Núcleo ao NORTE indo para L (tangencial)", -24.2, -50.5, 91, 40);
check("3. Tangencial (N indo pra L) = crossing", c3.v.approach === "crossing", `deu ${c3.v.approach}`);
...
const c4 = describe("Ponta Grossa → 200° SSW (passa de lado, NÃO chega)", -25.095, -50.158, 200, 30, false);
check("4. PG a 200° = crossing (passa de lado)", c4.v.approach === "crossing", `deu ${c4.v.approach}`);
...
const guaruja = projectCell(IPIRANGA.lat, IPIRANGA.lon, { directionDeg: 76, speedKmh: 556 }, 60);
const c5 = describe("CASO GUARUJÁ (556 km, 327° a 11,4 km/h)", guaruja.lat, guaruja.lon, 327, 11.4, false);
check("5a. Guarujá = crossing (não 'approaching')", c5.v.approach === "crossing", `deu ${c5.v.approach}`);
check("5b. Guarujá sem ETA (não inventa chegada)", c5.v.etaMin === null, `deu ${c5.v.etaMin}`);
check("5c. Guarujá na zona monitor (não vira boletim)", c5.zona === "monitor", `deu ${c5.zona}`);
check(
...
check("5e. distância medida ≈ 556 km", Math.abs(c5.d - 556) < 2, `deu ${c5.d.toFixed(1)}`);
...
const direto = projectCell(IPIRANGA.lat, IPIRANGA.lon, { directionDeg: 100, speedKmh: 60 }, 60);
const c6 = describe("CONTROLE: 60 km vindo direto (deve ALERTAR)", direto.lat, direto.lon, 280, 40, false);
check("6a. Núcleo vindo direto = approaching", c6.v.approach === "approaching", `deu ${c6.v.approach}`);
check("6b. ETA ~90 min", c6.v.etaMin != null && Math.abs(c6.v.etaMin - 90) < 2, `deu ${c6.v.etaMin}`);
check("6c. Zona alert", c6.zona === "alert", `deu ${c6.zona}`);

// 7. ETA absurdo: 200 km a 5 km/h = 40 h. Não é ETA → sem número, monitor.
const c7 = describe("200 km a 5 km/h (ETA de 40 h = absurdo)", -25.1, -52.5, 91, 5, false);
...
const c8 = describe("Rumo INVERTIDO (flag reversal)", -25.1, -52.5, 91, 40, false);
const vRev = assessThreat(-25.1, -52.5, { directionDeg: 91, speedKmh: 40, reversal: true }, IPIRANGA.lat, IPIRANGA.lon);
check("8a. Vetor com reversal = crossing", vRev.approach === "crossing", `deu ${vRev.approach}`);
...
function celula(lat: number, lon: number, mv: RainCell["trackedMovement"]): RainCell {
	return {
		intensity: "extreme",
		pixelCount: 5000,
		maxDbz: 55,
...
const mv = (dir: number, fromLat: number, fromLon: number, toLat: number, toLon: number) => ({
	directionDeg: dir,
	speedKmh: 40,
	intervalMin: 10,
	dxPx: 4,
...
function frames(celC: RainCell): FrameAnalysis[] {
	...
	const B = { time: 600_000, cells: [celula(0, 0.01, mv(90, 0, 0, 0, 0.01))], maxDbz: 55, coverage: 0.1 };
...
markReversals(fRev);
check("9a. Rumo invertido (90° → 270°) = reversal", fRev[2].cells[0].trackedMovement?.reversal === true);
check(
...
const fOk = frames(ok);
markReversals(fOk);
check("9c. Rumo consistente (90° → 90°) = confirmed", fOk[2].cells[0].trackedMovement?.confirmed === true);
check("9d. Rumo consistente NÃO tem flag de reversal", !fOk[2].cells[0].trackedMovement?.reversal);
...
markReversals(fSem);
check("9e. Sem par anterior = confirmed false", fSem[2].cells[0].trackedMovement?.confirmed === false);
check("9f. Sem par anterior NÃO é reversal", !fSem[2].cells[0].trackedMovement?.reversal);
