import type { PortalResult, ConnectivityResult, BgpResult, OperatorName } from "./types"
...
self.onmessage = async (event: MessageEvent<WorkerInput>) => {
  ...
  try {
    ...
    const res = await fetch(url, { signal: AbortSignal.timeout(input.bgpTimeoutMs) })
  ...
  const output: WorkerOutput = {
    ...
    connectivityResults: connectivityResults.map((r: Record<string, unknown>) => ({
