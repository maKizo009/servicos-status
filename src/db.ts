...
import type { PortalResult, ConnectivityResult, BgpResult } from "./types"
...
export function saveBgpResult(r: BgpResult): void {
  db.run(
    ...
    [r.operator, r.asn, r.prefixCountV4, r.prefixCountV6, r.timestamp, r.error ?? ""],
  )
}
...
export function getPortalHistory(operator: string, limit = 100): PortalResult[] {
  return db
    .query(
      `SELECT operator, host, success, latency_ms as latencyMs, error, timestamp
       FROM portal_results
